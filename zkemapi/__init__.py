...
def decode_time(t):
    ...
    d = str(year)+"-"+str(month).zfill(2)+"-"+str(day).zfill(2)+" "+str(hour).zfill(2)+":"+str(minute).zfill(2)+":"+str(second).zfill(2)
...
class zkem(object):
    ...
    def unpack_attendance_log(self):
        ...
        if len(self.__logdata) > 0:
            ...
            for i in l:
                uid = unpack('9s', i[2:11])[0]
                uid = uid.strip("\x00")
                state = unpack( 'B' , i[26:27] )[0]
                timestamp = decode_time( unpack( 'I' , i[27:31] )[0] )
                mode = unpack('B', i[31:32])[0]
                # valores para state y mode
                # state: 1 reconcimiento por medio de huella
                #        15 ... por medio del rostro
                # mode: 0 si es entrada
                #       1 si es salida
                log.append([uid, timestamp, state, mode])
        return log
